...
if(tvalue)attrs+="value=\""+tvalue+"\" ";//modify ' to ", for passing orgTree json data usage----xm
...
//add by xm. customized for uum orgTree usage. if the tree is orgTree, check parent node, will no check all children.
//check children, will not partially check parent node.
var root = $("ul[class='tree treeFolder treeCheck collapse'][id='orgTree']");
...
return false;}
function divSearch(form,rel,callback){
var $form=$(form);
if(form[DWZ.pageInfo.pageNum])form[DWZ.pageInfo.pageNum].value=1;
if(rel){
var $box=$("#"+rel);
//add a parameters "callback" which can specify user-defiend callback function ---by xumian
$box.ajaxUrl({
type:"POST",url:$form.attr("action"),data:$form.serializeArray(),callback:function(json) { callback(json, $box);}});}//function(){
...
html='<input type="hidden" name="'+field.lookupGroup+'.id'+suffix+'"/>'+'<input type="text" name="'+field.name+'"'+suggestFrag+' size="'+field.size+'" class="'+field.fieldClass+'"/>'+'<a class="btnLook" href="'+field.lookupUrl+'" lookupGroup="'+field.lookupGroup+'" '+suggestFrag+' title="查找带回">查找带回</a>';
break;
case 'attach':
html='<input type="hidden" name="'+field.lookupGroup+'.id'+suffix+'"/>'+'<input type="text" name="'+field.name+'" size="'+field.size+'" readonly="readonly" class="'+field.fieldClass+'"/>'+'<a class="btnAttach" href="'+field.lookupUrl+'" lookupGroup="'+field.lookupGroup+'" '+suggestFrag+' width="560" height="300" title="查找带回">查找带回</a>';
break;
case 'enum':
$.ajax({
