		...
		return this.each(function(){
			...
			$trs.hoverClass().each(function(){
				...
				$tr.click(function(){
					...
					var mTarget = $tr.attr("muliTarget");
					var mRel = $tr.attr("muliRel");
					if (mTarget && mRel) {
						var targetArr = mTarget.split(',');
						var relArr = mRel.split(',');
						if (targetArr.length!=relArr.length) {
							return
						}
						for(var i = 0, l = targetArr.length; i < l; i++) {
							if ($("#"+targetArr[i], $grid).size() == 0) {
								$grid.prepend('<input id="'+targetArr[i]+'" type="hidden" />');
							}
							$("#"+targetArr[i], $grid).val($tr.attr(relArr[i]));
						}
					}
				});
			});
