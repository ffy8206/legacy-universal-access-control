...
(function($){
	$.fn.extend({
		custompagination: function(opts){
			var setting = {
			...
			return this.each(function(){
				var $this = $(this);
				var pc = new CustomPagination(opts);
				...
				$this.find(setting.jumpto$).each(function(){
					...
					$button.click(function(event){
						...
						if (pageNum && pageNum.isPositiveInteger()) {
							customPageBreak({targetType:pc.targetType(), rel:pc.rel(), data: {pageNum:pageNum}});
						}
					});
			...
			function _bindEvent($target, pageNum, targetType, rel){
				$target.bind("click", {pageNum:pageNum}, function(event){
					customPageBreak({targetType:targetType, rel:rel, data:{pageNum:event.data.pageNum}});
		...
		orderBy: function(options){
			...
			return this.each(function(){
				var $this = $(this).css({cursor:"pointer"}).click(function(){
					...
					customPageBreak({targetType:op.targetType, rel:op.rel, data:{orderField: orderField, orderDirection: orderDirection}});
				});
	...
	var CustomPagination = function(opts) {
		this.opts = $.extend({
